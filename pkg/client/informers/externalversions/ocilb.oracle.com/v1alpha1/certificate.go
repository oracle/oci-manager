	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.OcilbV1alpha1().Certificates(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.OcilbV1alpha1().Certificates(namespace).Watch(options)
			},
		},
		&ocilb_oracle_com_v1alpha1.Certificate{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *certificateInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&ocilb_oracle_com_v1alpha1.Certificate{}, f.defaultInformer)
}
