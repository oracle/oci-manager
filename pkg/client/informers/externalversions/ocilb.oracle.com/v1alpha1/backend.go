	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.OcilbV1alpha1().Backends(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.OcilbV1alpha1().Backends(namespace).Watch(options)
			},
		},
		&ocilb_oracle_com_v1alpha1.Backend{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *backendInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&ocilb_oracle_com_v1alpha1.Backend{}, f.defaultInformer)
}
