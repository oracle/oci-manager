	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.CloudV1alpha1().LoadBalancers(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.CloudV1alpha1().LoadBalancers(namespace).Watch(options)
			},
		},
		&cloud_k8s_io_v1alpha1.LoadBalancer{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *loadBalancerInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&cloud_k8s_io_v1alpha1.LoadBalancer{}, f.defaultInformer)
}
